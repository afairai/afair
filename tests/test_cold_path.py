...
def test_stop_aborts_pending_batch(tmp_path: Path, settings_local: Settings) -> None:
    """A due batch checks the stop signal between workers: after stop(),
    _run_due_batch must not invoke anything."""
    worker = _CountingWorker()
    ...
    try:
        ...
        sched.stop()  # never started — just raises the stop flag
        sched._run_due_batch(conn, [worker])
