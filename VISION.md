...
**Managed (paying user).** Each user gets a dedicated machine, provisioned automatically. Same binary, same SQLite, operated by us. LiteFS for backup and optional read replication. The machine belongs to one user; if they cancel, the machine is destroyed (after retention window). Per-user dedicated URL provisioned via DNS API.
