...
**Status (2026-05-26):** Track 1 — Emergent Entity Graph — is shipped as the substrate-level mechanism behind the Emergent-Ontology track. Five new append-only tables (`entities`, `entity_mentions`, `entity_edges`, `entity_merges`, `edge_invalidations`) materialize a cross-event entity graph from the Extractor's per-event output. The `EntityCanonicalizer` cold-path worker resolves surface forms via a three-stage matcher (exact → LLM with lexical pre-pruning → new entity), with Sonnet escalation on Haiku low-confidence verdicts. `remember(invalidates=)` cascades into `edge_invalidations` so superseded edges hide from default recall. `recall` enriches each hit with a `canonical_entities` + `entity_edges` overlay inside the existing `interpretation` dict (no MCP-surface change). Entity-aware query routing auto-detects when a query string matches a canonical name or surface form and boosts matching events via reciprocal-rank fusion alongside FTS + vector. One-shot `scripts/backfill_entities.py` populates the graph for pre-existing vaults.

**Track 2 v0 (2026-05-26):** Per-hit surprise score shipped as a first observable slice of Active-Inference Salience. `RecallHit.interpretation.surprise_score ∈ [0, 1]` is computed as `(novel_canonical_entities / total_canonical_entities)` measured against the user's "recent context window" — the resolved canonical entities mentioned in the last N events (default 20, configurable via `SURPRISE_CONTEXT_WINDOW`). Hits whose entities all live in recent context score 0.0 (reinforcing); hits whose entities are absent from recent context score 1.0 ("comes out of nowhere"). Surfaced alongside `surprise_components` (novel_count, total_count, window_size) for AI-client auditability. Pure observability — the score is exposed so AI clients can self-route on it. The deeper Track 2 deliverables (CEN↔DMN mode-switching agent, surprise-triggered auto-`observe()`) remain future work pending Phase 2's Salience agent.
