...
The product model is not SaaS. It is software anyone can self-host, with an optional managed deployment where every paying user gets a dedicated isolated machine — one user, one machine, one SQLite database. Never multi-tenant. Never shared infrastructure for user data.
...
- **Per-user dedicated infrastructure is economically feasible.** [Fly.io](https://fly.io) puts single-tenant deployment at $3–8/month per user instance for typical workloads. Combined with continuous SQLite replication ([Litestream](https://litestream.io) → S3-compatible object store), the architecture that used to be reserved for premium tools (Fastmail, 1Password) is now viable for individual products.
...
**Managed (paying user).** Each user gets a dedicated machine, provisioned automatically. Same binary, same SQLite, operated by us. **Litestream** continuously streams the SQLite WAL to a per-user Cloudflare R2 bucket (~1-second RPO, hourly snapshots, 30-day WAL retention, EU residency). Recovery from any single-machine loss is automatic on boot. LiteFS (multi-region read replicas) is a future option when latency-bound regions appear; not required for Phase 0–1. The machine belongs to one user; if they cancel, the machine is destroyed (after retention window). Per-user dedicated URL provisioned via DNS API.
