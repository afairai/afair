        ...
        while not self._stop.wait(self._poll_seconds):
            ...
            if conn is None:
                try:
                    ...
                    continue
            for worker in due:
                # Re-check between workers, not just between cycles: a batch
                # of due workers can occupy the thread for minutes (a single
                # living_syntheses pass writes several syntheses), and a stop
                # that only lands after the batch finished is not a stop
                # button. One Event check per worker is a rounding error
                # next to an LLM call.
