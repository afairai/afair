...
`recall(decide=)` (operator conflict resolution: ADR-0008), and operator
content correction shipped on the same surface: mark a wrong source
superseded and suppress a wrong synthesis key point, both append-only
(ADR-0009). Broad distribution
...
| `docs/adr/ADR-0011-membership-federation.md` | Membership federation design of record (build deferred to v2; amended 2026-08-17 after a second adversarial review, 13 findings folded in place): one person across many sovereign single-tenant vaults. F1 personal-vault-as-gateway (OAuth rotating-refresh membership credentials, fan-out privilege only on rotating client credentials, always-available direct connect, fault-isolated recall fan-out serving an additive partial-results block; fan-out persistence left open with an ephemeral lean). F3 write topology: the model NEVER chooses a destination, and through the gateway writes ALWAYS land in the personal vault, with publication into a shared vault an explicit operator act on the Mirror; org-credentialed direct connect stays the frictionless shared path at the price of write monogamy; `observe` always pins to the personal vault; no routing argument is ever added to the wire surface. The two-tier confidentiality model (Tier 2 guarantee is gateway-scoped and true, session-scoped separation needs client-side connector hygiene afair cannot enforce and is named as the residual; Tier-1 co-residency implies mutual trust, orgs may mark a grant Tier-2-only; gateway recall honesty is scoped to reachable vaults and never hints at Tier 2). The Tier-1 content-laundering mitigation is span-level provenance matching on served bytes (rewording and ordering bypasses named). Federated write provenance requires an ADR-0006 amendment (gateway as semi-trusted forwarding tier) before build. F5 billing-only sponsored seat (person owns, org pays, employer reach refused on the record), with offboarding residue named as a permanent cost. F2/F4 (machine per vault; invite-first grants with cascading revocation required alongside cascading grants; the invite/grant/membership protocol is open core and vault-to-vault, control plane a convenience broker only, so self-hosted federation stays first-class per I4) | When the federation/membership model changes |
...
| `afair/agents/living_syntheses.py` | Deterministic emergent cluster discovery followed by model-written, cited synthesis. Combines entity recurrence, strong Binder links, and explicit lineage; suppresses mature hubs; preserves cluster identity and split/merge ancestry; replaces entity articles in the scheduler without breaking legacy reads. b3 (ADR-0009 Addendum 2026-07): before re-synthesizing a cluster it gathers the operator-marked-wrong claims (via `content_corrections.read_live_suppressions_for_steering`, function-scoped import to break the cycle) and appends a fenced steering block + static `_STEERING_RULE`; the no-suppression prompt stays byte-identical | When automatic cluster discovery, synthesis, or the b3 steering changes |
