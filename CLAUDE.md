...
| `docs/adr/ADR-0011-membership-federation.md` | Membership federation design of record (build deferred to v2): one person across many sovereign single-tenant vaults. F1 personal-vault-as-gateway (OAuth rotating-refresh membership credentials, always-available direct connect, fault-isolated recall fan-out); F3 credential-pinned write routing (the model NEVER chooses a destination; private-by-default anchored on the credential, not a mutable client default); the two-tier confidentiality model (convenience tier rides the gateway, confidential tier is direct-connect only so containment is structural); the graduated Tier-1 content-laundering mitigation (overlap-gated hold-and-surface, defense-in-depth with Tier 2 as the guarantee); F5 billing-only sponsored seat (person owns, org pays, employer reach refused on the record); F2/F4 (machine per vault; invite-first grants with cascading revocation required alongside cascading grants) | When the federation/membership model changes |
