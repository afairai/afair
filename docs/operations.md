...
- **App:** `afair-solis-e03` on Fly (the operator's per-user vault; every user gets one, single-tenant per I8)
...
- **URL:** `https://afair-solis-e03.fly.dev` (HTTPS auto-provisioned), MCP at `https://mcp.afair.ai`
...
`scripts/check_secrets.py`, deploys to `afair-solis-e03`, and verifies
`/health`.
...
```bash
URL=https://afair-solis-e03.fly.dev TOKEN=$(grep '^AFAIR_AUTH_TOKEN=' .env.local | cut -d= -f2-) \
...
| `fly apps destroy afair-solis-e03` | ❌ Everything gone — app, machine, volume, snapshots. |
...
fly ssh sftp shell -a afair-solis-e03 <<'EOF'
...
```bash
fly volumes list -a afair-solis-e03
# Note the volume id (vol_...).

fly machine list -a afair-solis-e03
...
fly machine create \
  ...
  --image registry.fly.io/afair-solis-e03:latest
```
...
```bash
fly volumes snapshots list -a afair-solis-e03
...
fly scale count 0 -a afair-solis-e03
...
fly volumes destroy <current-vol-id> -a afair-solis-e03
fly volumes create vault --region fra --size 1 --snapshot-id <snapshot-id>

# 4. Scale back up
fly scale count 1 -a afair-solis-e03
```
...
```bash
fly volumes list -a afair-solis-e03                # note volume id
fly volumes update <vol-id> -a afair-solis-e03 --snapshot-retention 14
```
...
2. Update Fly: `fly secrets set ANTHROPIC_API_KEY=... -a afair-solis-e03`
...
set in Fly (verified via `fly secrets list -a afair-solis-e03`) but their
...
fly secrets set AFAIR_AUTH_TOKEN="$NEW" -a afair-solis-e03
...
curl https://afair-solis-e03.fly.dev/health
# Expected: {"status":"ok"}
...
fly ssh console -a afair-solis-e03 -C "sqlite3 /data/vault/substrate.db 'SELECT COUNT(*) FROM events;'"
```
...
fly ssh console -a afair-solis-e03
...
fly machine restart -a afair-solis-e03

# 4. Run the backfill — populates the empty graph from existing events
fly ssh console -a afair-solis-e03 -C \
...
### `/health` returns 503
Run `fly logs -a afair-solis-e03`. Most common cause is the substrate DB being
...
# Set the Fly secret.
fly secrets set AFAIR_VAULT_KEY="$key" -a afair-solis-e03
...
fly volumes snapshots create <volume-id> -a afair-solis-e03

# 3. Stage the secret BEFORE pushing the encryption-layer code:
fly secrets set --stage AFAIR_VAULT_KEY=<value> -a afair-solis-e03
...
#    encrypted DB:
fly machine restart <machine-id> -a afair-solis-e03
